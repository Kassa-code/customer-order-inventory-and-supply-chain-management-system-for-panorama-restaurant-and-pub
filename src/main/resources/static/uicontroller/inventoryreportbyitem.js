...
const generateReport = () => {
  ...
  let dataList = getServiceRequest("/reportdailyingredientusage/bysdedtype?startdate=" + dteStartDate.value + "&enddate=" + dteEndDate.value + "&type=" + selectType.value);
  ...
  for (const index in dataList) {
     ...
     object.itemname = dataList[index][1];   // Month name
     object.amount = dataList[index][2];  // Amount value
  ...
  let propertyList = [
      ...
      { propertyName: "itemname", dataType: "string" },  // Column for month
  ...
  fillDataIntoReportTable(tableBodyInventoryByItemReport, reportDataList, propertyList);
