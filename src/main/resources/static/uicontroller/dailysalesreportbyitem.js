...
const generateReport = () => {
  ...
  let dataList = getServiceRequest("/reportdailysalesbyitem/bysdedtype?startdate=" + dteStartDate.value + "&enddate=" + dteEndDate.value + "&type=" + selectType.value);
  ...
  for (const index in dataList) {
    ...
    object.name = dataList[index][1];   // Month name
    object.total_completed_qty = dataList[index][2];  // Amount value
    object.total_line_price = dataList[index][3];  // Amount value
    reportDataList.push(object);         // Push into report list for table rendering

    label.push(dataList[index][1]);      // Add to label array for chart X-axis
    data.push(dataList[index][2]);       // Add to data array for chart Y-axis
  }
  ...
  let propertyList = [
    ...
    { propertyName: "name", dataType: "string" },  // Column for month
    { propertyName: "total_completed_qty", dataType: "string" },  // Column for month
    { propertyName: "total_line_price", dataType: "string" }, // Column for amount
  ];

  // Call the function to render the data in the HTML table
  // Parameters: tableBodyId, dataList, propertyList
  fillDataIntoReportTable(tableBodyDailySalesReportByItem, reportDataList, propertyList);
...
const printChart = () => {
  ...
  const printView = `
   <html>
     <head>
       <title>Print Customer Payment By Cash Report</title>
